...
| [ADR-012](./ADR-012-clustering-ab-schwelle.md) | Clustering erst ab 300 Treffern — mit Leaflets Standardfarben | vorgeschlagen | Ab 300 angezeigten Restaurants fasst `markercluster` die Marker zu Blasen zusammen, darunter bleibt alles wie heute (Standardbild unangetastet, DOM 81). Bei 885 gemessen: Frames über 100 ms von 9/12 auf **0/142**, `render()` 386 → 68 ms. Culling war wertlos (618 von 885 im Bild), Canvas hätte ADR-011 umgekehrt. Die Blasen behalten Leaflets Farben — **bewusste Ausnahme von ADR-009**, gegen die Empfehlung von [A-6](../anforderungen/A-6-clustering-oder-canvas.md) entschieden. |
| [ADR-013](./ADR-013-ergebnisliste-ist-der-barrierefreie-hauptweg.md) | Die Ergebnisliste ist der barrierefreie Hauptweg — Karte wird für Tastatur und Screenreader dekorativ | vorgeschlagen | Die Karte war nie „leer": 884 von 894 Tab-Stopps sind Marker, alle „Marker" genannt, und der Inhalt des eigenen offenen Popups liegt 884 Tabs entfernt (`popupPane` hinter `markerPane`). Die Liste zeigt dieselben Treffer nach Entfernung sortiert und mit den Zuständen in Worten; die Marker bekommen `keyboard: false` und `alt: ""` — benannte Ausnahme von ADR-011s Wortlaut, dessen Gegenstand (das Aussehen) unberührt bleibt. Mobil Sheet, ab 640 px dauerhaftes Panel (75 % Kartenbreite bei 1280 px). Gebaut mit [A-2](../anforderungen/A-2-ergebnisliste.md). |
