...
- **A-1 (Standardfilter) is decided and shipped** ([ADR-007](docs/entscheidungen/ADR-007-standardfilter-liefert-jetzt.md), 2026-07-25): the default filter is **„Liefert jetzt"** — `delivery` **and** currently open, ~35 of 883 at midday and zero at night, deliberately narrower than before. The map is a *Jetzt-Werkzeug, kein Verzeichnis*. Read the ADR before touching `FILTER_DEFAULTS`; widening the default back out is a reversal that needs a new ADR, not an edit. A-8 (filters in the URL) shipped with it. The three requirements A-1 blocked are unblocked: A-5 waited on A-4, which shipped the same day (see the colour-roles constraint below), so nothing blocks it now; A-6 (clustering) dropped back to „nice to have" because 883 markers only render when the user clears the filters.
