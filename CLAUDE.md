...
- **PWA (`web/sw.js` + `web/manifest.webmanifest`)** — installable and offline-capable, no build step. Paths are relative (`./…`) because Pages serves the site from `/<repo>/web/`. Details in `TECHNICAL.md` ("PWA"); design rationale in `IDEEN.md`.
